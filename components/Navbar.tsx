        ...
        <div className="flex items-center gap-1">
          ...
          {user ? (
            <>
              ...
              <button onClick={() => router.push("/settings")} className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-[var(--color-bg-hover)] transition-all duration-200">
              ...
              <div className="flex items-center gap-1.5 ml-1">
                ...
                <div className="w-6 h-6 rounded-full bg-gradient-to-br from-zinc-600 to-zinc-500 flex items-center justify-center text-white text-[10px] font-bold overflow-hidden">
                  {user.avatar ? <img src={user.avatar} alt="" className="w-full h-full object-cover" /> : user.name.charAt(0).toUpperCase()}
                </div>
              </div>
            </>
          ) : (
            <button onClick={requireLogin} className="btn-primary text-xs px-3.5 py-1.5 rounded-lg ml-1">登录</button>
          )}
        </div>
      </div>
    </header>
